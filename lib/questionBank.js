...
const GENERATORS = {
  ...
  // Expanded Algebra strand \u2014 all reuse slope-intercept / parallel-perp / modelling banks
  ...
  // Expanded Number Sense strand \u2014 all reuse rational-ops bank
...
export const TESTS = [
  {
    ...
    description: 'Warm up across all Grade 9 strands. Great for a quick check-in.',
    totalQuestions: 20,
    estMinutes: 10,
    weights: {
...
export function getTestQuestions(testId) {
  ...
  if (!t) return null;
  const items = [];
  const seedBase = testId === 'test-foundations' ? 500
                : testId === 'test-midterm' ? 1200
                : 2000;
